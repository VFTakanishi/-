...
- 起動メッセージ・エラー（Pythonが見つからない、依存パッケージや`ANTHROPIC_API_KEY`が不足している等）は画面には表示されず、`youtube-clipper\logs\launcher.log` に記録されます。起動しない場合はまずこのファイルを確認してください
- 終了するには、開いたウィンドウで`Ctrl+C`を押すか、ウィンドウを閉じてください

### デスクトップアイコンから起動する（コマンドプロンプトを一切見たくない場合）

`youtube-clipper` フォルダで一度だけ以下を実行してください（PowerShell）:

```powershell
powershell -ExecutionPolicy Bypass -File setup_desktop_shortcut.ps1
```

デスクトップに **「Podcast Clipper」** というショートカットが1つ作成されます。以降はこれをダブルクリックするだけで、黒い画面を一切表示せずにサーバーが起動し、ブラウザが自動で開きます。既に起動中の状態でもう一度ダブルクリックした場合は、新しいサーバーを起動せず既存のものをブラウザで開きます。

- Windowsのスタートアップ登録・タスクスケジューラ・レジストリ・サービス登録は一切行いません
- 元に戻すには、作成されたショートカットをデスクトップから削除するだけです
- 起動ログは`start.bat`と同じ `youtube-clipper\logs\launcher.log` に記録されます
