...
`candidates`: 設計した完成candidateのリスト（強い順。最大3件）。各candidateは以下を持つ:
...
**これが最終的に採用すべきベスト3です。** 意味的完結性・カット接続の自然さ・尺（20〜50秒）のいずれかを満たす設計を3件用意できない場合は、**無理に3件埋めず、条件を満たす件数だけを返してください**（0〜2件でも構いません。後段の処理が適切に扱います）。
